...
function Content({
  ...
  Section,
  'header' | 'subheader' | 'content' | 'buttonText'
>): React.ReactNode {
  return (
    <motion.div
      initial={{ opacity: 0 }}
    ...
    >
      <h1 className="text-lg font-extrabold text-blue uppercase mb-4">
        {header}
      </h1>
      <p className="text-3xl font-bold mb-3 lg:mb-5">{subheader}</p>
      <p className="text-lg leading-8 font-normal pb-12">{content}</p>
      <button className="px-5 py-2 border-[0.4px] border-solid border-gray mb-7">
        {buttonText}
      </button>
    </motion.div>
  );
        ...
        {images.map((image) => {
          return (
            <motion.img
              ...
              className={image.className}
              variants={
                image.animation.variants as Variants | undefined as Variants
              }
            />
          );
        })}
...
export default function HomepageView() {
  ...
  return (
    <div className="mt-14 lg:mt-32 ">
      ...
      <div className="w-full h-[500px] flex items-center  z-50 relative bg-white"></div>
    </div>
  );
}
